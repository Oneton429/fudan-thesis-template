...
众所不周知，维普的格式检测系统仅支持 Word 的格式。可以考虑导出 PDF 后，使用 Acrobat 转换为 Word 格式，并修改掉那些一眼是因为格式转换导致的错误（如引用附近的正文被莫名起名上标等），再进行格式检测，并且自行判断哪些是格式错误，哪些是格式转换导致的，哪些是检测系统处理导致的。

转换出来的正文一般是多倍行距（也有偶发固定值的时候），可以考虑手动转为 20 磅行距后再上传。
